TOKEN = "8297296486:AAGyMV3Mir10Ja0aXMIY2aFJGb13-n6keAI"
ADMIN_IDS = [5858391454]
...
def callback_handler(call):
    ...
    print(f"\n📨 CALLBACK получен: {call.data} от пользователя {user_id}")
    ...
    print(f"📊 Админ действие: {call.data}")
...
Реферальная награда: <code>{settings_data['referral_reward']} монет</code>
    ...
    # ОБРАБОТЧИК ПОДТВЕРЖДЕНИЯ РАССЫЛКИ
        ...
        print(f"🚀 ПОЛУЧЕНО ПОДТВЕРЖДЕНИЕ РАССЫЛКИ: {call.data}")
        
        # Немедленно подтверждаем нажатие
        try:
        ...
        if not state_data or state_data.get("state") != "confirm_broadcast":
            bot.edit_message_text("❌ <b>Ошибка: данные рассылки не найдены!</b>\n\n"
                                 "Попробуйте начать рассылку заново.", 
                                 user_id, message_id, parse_mode="HTML")
            return
        ...
        # ЗАПУСКАЕМ РАССЫЛКУ
        start_broadcast_directly(user_id, message_id, broadcast_text)
...
def handle_admin_states(message):
    ...
    if state == "awaiting_broadcast":
        ...
        broadcast_id = str(abs(hash(broadcast_text)) % 1000000)
...
def start_broadcast_directly(admin_id, message_id, broadcast_text):
    """Функция запуска рассылки напрямую"""
    print(f"\n" + "="*60)
    print(f"🚀 ЗАПУСК РАССЫЛКИ НАПРЯМУЮ")
    print(f"Админ: {admin_id}")
    print(f"Текст: {broadcast_text[:50]}...")
    print("="*60)
    
    all_users = users.all()
    total_users = len(all_users)
    
    print(f"📊 Пользователей для рассылки: {total_users}")
    
    if total_users == 0:
        try:
            bot.edit_message_text("❌ <b>Нет пользователей для рассылки!</b>\n\n"
                                 f"В базе данных нет ни одного пользователя.", 
                                 admin_id, message_id, parse_mode="HTML")
        except:
            bot.send_message(admin_id, "❌ <b>Нет пользователей для рассылки!</b>")
        return
    ...
    try:
        ...
        new_msg = bot.send_message(admin_id, f"<b>📢 Начинаю рассылку...</b>\n\n"
                                           f"Всего получателей: <code>{total_users}</code>")
    ...
    for i, user in enumerate(all_users, 1):
        ...
        try:
            print(f"[{i}/{total_users}] Отправляю пользователю {user_id_to_send}...")
            
            # ОСНОВНАЯ ОТПРАВКА
            bot.send_message(user_id_to_send, broadcast_text, parse_mode="HTML")
            ...
            if "blocked" in error_msg:
                blocked += 1
            elif "deactivated" in error_msg or "chat not found" in error_msg:
                blocked += 1
        ...
        # Обновляем прогресс каждые 3 отправки
        if i % 3 == 0 or i == total_users:
            try:
    ...
    # Завершаем рассылку
    user_states.remove(Query().id == admin_id)
    ...
    try:
        bot.edit_message_text(final_text, admin_id, message_id, 
                            parse_mode="HTML", reply_markup=markup)
    except Exception as e:
        print(f"Ошибка при завершении рассылки: {e}")
        bot.send_message(admin_id, final_text, reply_markup=markup)
    
    print(f"\n" + "="*60)
    print(f"✅ РАССЫЛКА ЗАВЕРШЕНА")
    print(f"Успешно: {success}/{total_users}")
...
def check_users_cmd(message):
    ...
    if total_users == 0:
        bot.send_message(message.chat.id, "❌ <b>В базе нет ни одного пользователя!</b>")
        return
    
    bot.send_message(message.chat.id, f"✅ <b>Пользователей в базе:</b> <code>{total_users}</code>")
    
    # Показываем первых 5 пользователей
    text = "<b>Первые 5 пользователей:</b>\n"
    for i, user in enumerate(all_users[:5], 1):
        text += f"{i}. ID: <code>{user['id']}</code> | @{user.get('username', 'нет')}\n"
...
def test_send_cmd(message):
    ...
    # Пробуем отправить себе
    try:
        bot.send_message(message.chat.id, "📨 <b>Тестовое сообщение</b>\n\nЭто тест отправки сообщения.")
        bot.send_message(message.chat.id, "✅ Сообщение себе отправлено успешно!")
    except Exception as e:
        bot.send_message(message.chat.id, f"❌ Ошибка отправки себе: {str(e)[:200]}")
    
    # Пробуем отправить первому пользователю
    if len(all_users) > 0:
        first_user = all_users[0]
        try:
            bot.send_message(first_user["id"], "📨 <b>Тестовое сообщение от бота</b>\n\nЭто тестовая рассылка.")
            bot.send_message(message.chat.id, f"✅ Отправлено пользователю {first_user['id']}")
        except Exception as e:
            bot.send_message(message.chat.id, f"❌ Ошибка отправки пользователю: {str(e)[:200]}")

@bot.message_handler(commands=["direct_broadcast"])
def direct_broadcast_cmd(message):
    """Прямая рассылка без кнопок"""
    if message.chat.id not in ADMIN_IDS:
        return
    
    msg = bot.send_message(message.chat.id, "📝 <b>Введите текст для прямой рассылки:</b>")
    bot.register_next_step_handler(msg, process_direct_broadcast)

def process_direct_broadcast(message):
    """Обработка текста для прямой рассылки"""
    broadcast_text = message.text
    
    if not broadcast_text or broadcast_text.strip() == "":
        bot.send_message(message.chat.id, "❌ Текст рассылки не может быть пустым!")
        return
    
    # Запускаем рассылку напрямую
    start_msg = bot.send_message(message.chat.id, f"🚀 <b>Запускаю прямую рассылку...</b>\n\n"
                                                 f"Текст: {broadcast_text[:100]}...")
    
    # Запускаем рассылку
    start_broadcast_directly(message.chat.id, start_msg.message_id, broadcast_text)
...
def run_bot():
    while True:
        try:
            ...
            # Проверка пользователей при запуске
            all_users = users.all()
