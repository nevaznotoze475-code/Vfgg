...
def handle_admin_states(message):
    ...
    if state == "awaiting_broadcast":
        ...
        markup = types.InlineKeyboardMarkup(row_width=2)
        markup.add(
            types.InlineKeyboardButton("✅ Начать рассылку", callback_data=f"confirm_broadcast_{hash(broadcast_text)}"),
            types.InlineKeyboardButton("❌ Отмена", callback_data="admin_back")
        )
        
        user_states.update({"state": "confirm_broadcast", "broadcast_text": broadcast_text}, Query().id == user_id)
        send_message(user_id, f"<b>📢 Подтверждение рассылки</b>\n\n"
                            f"Сообщение:\n"
                            f"<code>{broadcast_text[:100]}...</code>\n\n"
...
def handle_confirm_broadcast(call):
    ...
    bot.edit_message_text(f"<b>📢 Рассылка начата...</b>\n\n"
                         f"Отправлено: <code>0/{len(all_users)}</code>\n"
    ...
    for i, user in enumerate(all_users, 1):
        try:
            send_message(user["id"], broadcast_text)
            success += 1
        except Exception as e:
            errors += 1
        ...
        if i % 10 == 0 or i == len(all_users):
            try:
                bot.edit_message_text(f"<b>📢 Рассылка в процессе...</b>\n\n"
                                     f"Отправлено: <code>{i}/{len(all_users)}</code>\n"
    ...
    bot.edit_message_text(f"<b>✅ Рассылка завершена!</b>\n\n"
                         f"Всего получателей: <code>{len(all_users)}</code>\n"
